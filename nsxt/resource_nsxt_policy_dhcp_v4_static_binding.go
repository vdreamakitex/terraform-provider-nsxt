...
func policyDhcpV4StaticBindingConvertAndPatch(obj model.DhcpV4StaticBindingConfig, segmentID string, id string, m interface{}) error {
...
func resourceNsxtPolicyDhcpV4StaticBindingCreate(d *schema.ResourceData, m interface{}) error {
	...
	err = policyDhcpV4StaticBindingConvertAndPatch(obj, segmentID, id, m)
...
func resourceNsxtPolicyDhcpV4StaticBindingRead(d *schema.ResourceData, m interface{}) error {
	...
	if isPolicyGlobalManager(m) {
		client := gm_segments.NewDefaultDhcpStaticBindingConfigsClient(connector)
		gmObj, err := client.Get(segmentID, id)
		if err != nil {
			return handleReadError(d, "DhcpV4StaticBindingConfig", id, err)
		}

		lmObj, err := convertModelBindingType(gmObj, gm_model.DhcpStaticBindingConfigBindingType(), model.DhcpV4StaticBindingConfigBindingType())
		if err != nil {
			return err
		}
		obj = lmObj.(model.DhcpV4StaticBindingConfig)
	} else {
		client := segments.NewDefaultDhcpStaticBindingConfigsClient(connector)
		...
		lmObj, err := convertModelBindingType(dhcpObj, model.DhcpStaticBindingConfigBindingType(), model.DhcpV4StaticBindingConfigBindingType())
		if err != nil {
			return err
		}
		obj = lmObj.(model.DhcpV4StaticBindingConfig)
